...
[super load];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(initialize* NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
-(void)initTapGes
{
    ...
    tap.numberOfTapsRequired = 2; //taps
    tap.numberOfTouchesRequired = 3; //fingers
...
-(void)initTapGes2
{
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] init];
    tap.numberOfTapsRequired = 2; //taps
    tap.numberOfTouchesRequired = 2; //fingers
    [[UIApplication sharedApplication].windows[0].rootViewController.view addGestureRecognizer:tap];
